...
class ProfessorDoctorQuotaInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['subject', 'total_quota', 'used_quota', 'remaining_quota']
    readonly_fields = ['used_quota', 'remaining_quota']
    can_delete = False
...
class ProfessorAdmin(admin.ModelAdmin):
    fieldsets = [
        ("导师信息更改", {"fields": ["name", "teacher_identity_id", "professor_title", "email", "department",
                                     "enroll_subject", "academic_quota", "professional_quota", "professional_yt_quota",
                                     "doctor_quota", "proposed_quota_approved", "have_qualification", "remaining_quota",
                                     "personal_page", "research_areas", "avatar", "contact_details", "department_position"]}),
    ]
    list_display = ["teacher_identity_id", "name", "department", "have_qualification", "proposed_quota_approved"]
    readonly_fields = ["remaining_quota"]
    actions = [reset_quota, reset_proposed_quota_approved, 'reset_password_to_teacher_id']
    change_list_template = 'admin/professor_change_list.html'
    inlines = [ProfessorDoctorQuotaInline]

    def reset_password_to_teacher_id(self, request, queryset):
        for professor in queryset:
            if professor.user_name:
                teacher_id = professor.teacher_identity_id
                professor.user_name.set_password(teacher_id)
                professor.user_name.save()
                self.message_user(
                    request,
                    f"已重置导师 {professor.name} 的密码为工号: {teacher_id}",
    ...
    reset_password_to_teacher_id.short_description = "重置密码为工号"

    def get_actions(self, request):
        actions = super().get_actions(request)
        actions['reset_academic_quota'] = (reset_quota, 'reset_academic_quota', '重置学硕名额为 0')
        actions['reset_professional_quota'] = (reset_quota, 'reset_professional_quota', '重置北京专硕名额为 0')
        actions['reset_professionalyt_quota'] = (reset_quota, 'reset_professionalyt_quota', '重置烟台专硕名额为 0')
        actions['reset_doctor_quota'] = (reset_quota, 'reset_doctor_quota', '重置博士名额为 0')
        actions['reset_proposed_quota_approved'] = (reset_proposed_quota_approved, 'reset_proposed_quota_approved',
                                                   '重置所选导师状态为未开放选择')
        return actions
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    required_columns = ["工号", "姓名", "学术学位硕士", "专硕北京", "专硕烟台", "博士"]
                    if not all(column in reader.fieldnames for column in required_columns):
                        self.message_user(request, "CSV 文件列名不正确，请确保包含：工号、姓名、学术学位硕士、专硕北京、专硕烟台、博士",
                                         level='error')
                        return redirect('admin:Professor_Student_Manage_professor_changelist')
    ...
    def get_readonly_fields(self, request, obj=None):
        readonly_fields = self.readonly_fields
        if obj and obj.proposed_quota_approved:
            readonly_fields = ["have_qualification", "remaining_quota"]
        return readonly_fields
...
class StudentAdmin(admin.ModelAdmin):
    fieldsets = [
        ("学生信息更改", {"fields": ["name", "candidate_number", "student_type", "subject",
                               ...
                               "is_selected", "is_giveup"]}),
    ]
    list_display = ["candidate_number", "name", "subject", "study_mode", "student_type", "postgraduate_type",
                    "is_selected", "is_giveup", "render_hx_download_link", "render_fq_download_link"]
    list_filter = ["subject"]
    search_fields = ["name"]
    actions = ['reset_password_to_exam_id']
    change_list_template = 'admin/student_change_list.html'
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    if not all(column in reader.fieldnames for column in required_columns):
                        self.message_user(request, "CSV 文件列名不正确，请确保包含：准考证号、姓名、学生类型、研究生类型、学习方式",
                                         level='error')
                        return redirect('admin:Professor_Student_Manage_student_changelist')
                    ...
                    for row in reader:
                        ...
                        try:
                            if Student.objects.filter(candidate_number=candidate_number).exists():
                                self.message_user(request, f"考生编号 {candidate_number} 已存在，跳过此记录",
                                                 level='warning')
                                continue

                            username = candidate_number
                            if User.objects.filter(username=username).exists():
                                self.message_user(request, f"用户名 {username} 已存在，跳过此记录",
                                                 level='warning')
                                continue
                            ...
                            self.message_user(request, f"准考证号 {candidate_number} 的数据格式不正确: {str(e)}",
                                             level='warning')
                            continue
                        except Exception as e:
                            self.message_user(request, f"创建学生 {candidate_number} 时出错: {str(e)}",
                                             level='error')
                            continue
    ...
    def reset_password_to_exam_id(self, request, queryset):
        for student in queryset:
            if student.user_name:
                candidate_number = student.candidate_number
                student.user_name.set_password(candidate_number)
                student.user_name.save()
                self.message_user(
                    request,
                    f"已重置学生 {student.name} 的密码为准考证号: {candidate_number}",
    ...
    reset_password_to_exam_id.short_description = "重置密码为准考证号"
...
class DownloadFileView(APIView):
    ...
    def post(self, request):
        logger.info(f"DownloadFileView called with data: {request.data}")
        file_id = request.data.get('file_id')
        if not file_id:
            logger.error("Missing file_id parameter")
            return Response({'message': '缺少 file_id 参数'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            response = requests.post(url, json=data)
            response_data = response.json()
            logger.info(f"WeChat API response: {response_data}")
            if response_data.get("errcode") == 0:
                download_url = response_data['file_list'][0]['download_url']
                return Response({'download_url': download_url}, status=status.HTTP_200_OK)
            else:
                logger.error(f"WeChat API error: {response_data}")
                return Response({'message': '获取下载地址失败'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            logger.error(f"Server error in DownloadFileView: {str(e)}")
            return Response({'message': f'服务器错误: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
