...
export default function MasterQuotasPage() {
  ...
  const fetchData = async (
    ...
    try {
      ...
      const payload = await get(`/master-quotas/?${params.toString()}`);
      setData(payload);
  ...
  const openEditModal = (record) => {
    ...
    form.setFieldsValue({
      ...
      beijing_quota: record.beijing_quota,
      beijing_remaining_quota: record.beijing_remaining_quota,
      yantai_quota: record.yantai_quota,
      yantai_remaining_quota: record.yantai_remaining_quota,
    });
    setEditOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = editingRecord
        ? await patch(`/master-quotas/${editingRecord.id}/`, values)
        : await post('/master-quotas/', values);
  ...
  const handleDelete = (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const payload = await del(`/master-quotas/${record.id}/`);
  ...
  const columns = [
    ...
    { title: '硕士专业', dataIndex: 'subject_name', key: 'subject_name', sorter: true },
    { title: '专业代码', dataIndex: 'subject_code', key: 'subject_code', sorter: true },
    { title: '专业类型', dataIndex: 'subject_type_display', key: 'subject_type_display' },
    { title: '北京名额', dataIndex: 'beijing_quota', key: 'beijing_quota', sorter: true },
    { title: '北京剩余', dataIndex: 'beijing_remaining_quota', key: 'beijing_remaining_quota', sorter: true },
    { title: '烟台名额', dataIndex: 'yantai_quota', key: 'yantai_quota', sorter: true },
    { title: '烟台剩余', dataIndex: 'yantai_remaining_quota', key: 'yantai_remaining_quota', sorter: true },
  ...
  return (
    <>
      <Card className="page-card" bordered={false}>
        <Typography.Title level={3}>导师硕士专业名额</Typography.Title>
        <div className="toolbar" style={{ gap: 12, flexWrap: 'wrap' }}>
          ...
          <Select
            ...
            onChange={(value) => updateFilter('subject_type', value)}
          />
          <Space wrap>
      ...
      <Modal
        title={editingRecord ? '编辑导师硕士专业名额' : '新建导师硕士专业名额'}
        open={editOpen}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="subject_id" label="硕士专业" rules={[{ required: true, message: '请选择硕士专业' }]}>
            <Select
              {...selectSearchProps}
              allowClear
          ...
          <Form.Item name="beijing_quota" label="北京名额" rules={[{ required: true, message: '请输入北京名额' }]}>
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="beijing_remaining_quota" label="北京剩余名额">
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="yantai_quota" label="烟台名额" rules={[{ required: true, message: '请输入烟台名额' }]}>
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="yantai_remaining_quota" label="烟台剩余名额">
            <InputNumber min={0} style={{ width: '100%' }} />
          </Form.Item>
        </Form>
      </Modal>
